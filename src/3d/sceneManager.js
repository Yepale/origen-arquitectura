...
/** ORIGEN — Scene Manager v2
 * Winter Sierra atmosphere, warm sunrise, high-contrast stone readability.
 */
export class SceneManager {
  constructor(canvas){
    this.canvas=canvas;this.scene=null;this.camera=null;this.renderer=null;this.particles=null;this.lights={};this.clock=new THREE.Clock();this.animationCallbacks=[];this.isDisposed=false;this.pedestalGroup=null;this.init();
  }
  init(){
    this.scene=new THREE.Scene();
    const loader=new THREE.TextureLoader();
    const applyBackground=(texture)=>{texture.colorSpace=THREE.SRGBColorSpace;texture.mapping=THREE.UVMapping;this.scene.background=texture;this.scene.environment=null};
    loader.load('/images/origen_panoramic_background.jpg',applyBackground,undefined,()=>{this.scene.background=new THREE.Color(0x111318)});
    ...
    this.camera=new THREE.PerspectiveCamera(42,aspect,.1,100);this.camera.position.set(0,.25,8.25);this.camera.lookAt(0,-.1,0);
    this.renderer=new THREE.WebGLRenderer({canvas:this.canvas,antialias:true,powerPreference:'high-performance',alpha:false});
    this.renderer.setSize(this.canvas.clientWidth||window.innerWidth,this.canvas.clientHeight||window.innerHeight,false);this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,2));
    this.renderer.shadowMap.enabled=true;this.renderer.shadowMap.type=THREE.PCFSoftShadowMap;this.renderer.toneMapping=THREE.ACESFilmicToneMapping;this.renderer.toneMappingExposure=1.15;
  ...
  setupLighting(){
    ...
    const sun=new THREE.DirectionalLight(0xffd39a,2.7);sun.position.set(5,6,4);sun.castShadow=true;sun.shadow.mapSize.width=2048;sun.shadow.mapSize.height=2048;sun.shadow.camera.near=.5;sun.shadow.camera.far=35;sun.shadow.camera.left=-8;sun.shadow.camera.right=8;sun.shadow.camera.top=8;sun.shadow.camera.bottom=-8;sun.shadow.bias=-.0003;this.scene.add(sun);this.lights.sun=sun;
  ...
  setupDustParticles(){
    const count=140,g=new THREE.BufferGeometry(),positions=new Float32Array(count*3),vel=[];
    ...
    this.particles=new THREE.Points(g,new THREE.PointsMaterial({size:.095,map:new THREE.CanvasTexture(c),transparent:true,opacity:.28,blending:THREE.AdditiveBlending,depthWrite:false}));this.particleVelocities=vel;this.scene.add(this.particles);
  }
