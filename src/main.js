...
import { SeasonController } from './season.js';
import gsap from 'gsap';

/** ORIGEN — Main Application & State Machine Orchestrator v2.2 */
class OrigenApp {
  constructor(){
    this.state='INTRO';this.sceneManager=null;this.piecesData=null;this.interaction=null;this.cameraRig=null;this.materialViewer=null;this.season=null;
  ...
  init(){
    ...
    this.season=new SeasonController(this.sceneManager);this.season.init();
    this.piecesData=buildStonePieces(this.sceneManager.scene);this.sceneManager.pedestalGroup=this.piecesData.pedestalGroup;
    ...
    this.setupUIEvents();this.bindSeasonControls();
  }
  bindSeasonControls(){
    document.querySelectorAll('[data-season]').forEach(btn=>btn.addEventListener('click',()=>this.season.setSeason(btn.dataset.season)));
    this.season.subscribe(()=>this.refreshSeasonButtons());this.refreshSeasonButtons();
    window.addEventListener('resize',()=>this.season.applyLandingBackground(this.season.current,false));
  }
  refreshSeasonButtons(){
    document.querySelectorAll('.season-control [data-season]').forEach(btn=>{const active=btn.dataset.season===this.season.current;btn.classList.toggle('active',active);btn.setAttribute('aria-pressed',String(active));});
  }
  setupUIEvents(){
  ...
  handleAllLocked(){this.state='COMPLETE';gsap.to(this.dom.introUi,{opacity:0,duration:.7,ease:'power2.out',pointerEvents:'none'});this.cameraRig.focusCompletedSymbol(()=>setTimeout(()=>this.startFlyThrough(),900));}
  startFlyThrough(){this.state='FLY_THROUGH';this.cameraRig.flyThrough(()=>{if(this.dom.transitionOverlay)gsap.to(this.dom.transitionOverlay,{opacity:1,duration:.4,ease:'power2.in',onComplete:()=>this.showLandingPage()});else this.showLandingPage()},()=>{if(this.dom.transitionOverlay)gsap.to(this.dom.transitionOverlay,{opacity:0,duration:.85,ease:'power2.out'})});}
  showLandingPage(){this.state='LANDING';if(this.dom.introExp)this.dom.introExp.style.display='none';if(this.dom.landingPage){this.dom.landingPage.classList.add('visible');this.dom.landingPage.setAttribute('aria-hidden','false');window.scrollTo(0,0);if(!this.materialViewer)this.materialViewer=new MaterialViewer('viewer-canvas');}}
  reviveIntro(){this.state='INTRO';if(this.dom.landingPage){this.dom.landingPage.classList.remove('visible');this.dom.landingPage.setAttribute('aria-hidden','true');}if(this.dom.introExp)this.dom.introExp.style.display='block';if(this.dom.introUi){this.dom.introUi.style.opacity='1';this.dom.introUi.style.pointerEvents='auto';}Object.values(this.piecesData.pieces).forEach(p=>{p.isLocked=false;p.isDragging=false;p.group.position.copy(p.initialPos);p.group.rotation.copy(p.initialRot);if(p.ghost)p.ghost.visible=false;if(p.glowMesh)p.glowMesh.visible=false;});this.interaction.lockedCount=0;this.interaction.isCompleted=false;this.cameraRig.resetToIntro();}
}
